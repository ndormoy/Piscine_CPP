...
bool	ScalarConversion::__print_all(char c, int i, float f, double d, int type) const
{
	if (std::isprint(c))
...
bool	ScalarConversion::__is_char(char c) const
{
	char	my_c = static_cast<char> (c);
	int		i = static_cast<int> (c);
	float	f = static_cast<float> (c);
	double	d = static_cast<double> (c);
	return (__print_all(my_c, i, f, d, CHAR));
}
...
bool	ScalarConversion::__is_int(std::string str) const
{
	...
	return (__print_all(c, i, f, d, INT));
}
